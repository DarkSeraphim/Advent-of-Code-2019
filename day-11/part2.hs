...
import Data.Map (Map, insert, findWithDefault, (!), fromList, empty, size, singleton, keys)
...
getPixel :: Integer -> Char
getPixel 0 = ' '
getPixel 1 = '#'

-- Rip me, my sign is flipped, let's flip the y range instead to fix that
rev :: [Integer] -> [Integer]
rev [] = []
rev (x:xs) = (rev xs) ++ [x]
...
main = do
  ...
  grid <- sendPaintBotCamera (0, 0) (0, 1) (singleton (0, 0) 1) stdio
  ...
  let rangeX = [0..(maxX - minX)]
  let rangeY = rev [0..(maxY - minY)]
  let text = [[getPixel (findWithDefault 0 ((x + minX), (y + minY)) grid) | x <- rangeX] | y <- rangeY]
  let sign = foldl (\acc next -> acc ++ "\n" ++ next) "" text
  putStr sign
